...
enzymes = ['ManI','GalT','GnT','SiaT', 'FucT'] #add starting enzymes. If there is no siaT there is no sialic acid
init_str = ('F%sM%sA%sG%sS%s') #starting input which will be F0M0A0G0S0
def N_glycoylation_pathway(enzymes):
    ...
    for i in range(0,len(mannose)) : #extract Mannose
        ...
        if m == 5 or m == 4: #append GlucNAc
            ...
            if a == 1:
                ...
                while g < 1 and 'GalT' in enzymes: #append Galactose
                    add_in =  init_str %(0,m,a,g,0)
        ...
        if m == 3: #if all manose is removed
            while a < m+1 and 'GnT' in enzymes: #append GluNac m+1 indicates that 2 GlcNAc append to 1 branch of mannose
                add_in =  init_str %(0,m,a,0,0)
                add_out = init_str %(0,m,a+1,0,0)
                reactions_in.append(add_in)
                reactions_out.append(add_out)
                ...
                if a != 0:
                    if g == 0 and 'GalT' in enzymes: 
                        ...
                        d['Glycan_out'].append(add_out)
                        d['Residue'].append('GDP')
                    while g < a and 'GalT' in enzymes: #append Galactose
                        add_in =  init_str %(0,m,a,g,0)
                        add_out = init_str %(0,m,a,g+1,0)
                        reactions_in.append(add_in)
                        reactions_out.append(add_out)
                        ...
                        s = 0    
                        if g != 0:
                            while s < g and 'SiaT' in enzymes: #append Sialic Acid
                                add_in =  init_str %(0,m,a,g,s)
                                add_out = init_str %(0,m,a,g,s+1)
                                reactions_in.append(add_in)
                                reactions_out.append(add_out)
        ...
        if 'FucT' in enzymes:
            ...
            if m < 5:
                ...
                s = 0    
                if g == 1:
                    while s < 1 and 'SiaT' in enzymes: #append Sialic acid
...
d = N_glycoylation_pathway(enzymes)       
